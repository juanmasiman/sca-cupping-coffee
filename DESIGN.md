...
**Advise from what you measured, not only from what you were told.** The dial-in withheld every suggestion until somebody had moved the taste slider. Three shots into a first session it had offered exactly one piece of guidance, and that piece was *"tap this shot and say how it tasted to get a next move"* — the app asking for work before it would help, while sitting on the answer. It had printed "6s under the window" and stopped.

Time is a measurement, not a guess, and grind is what changes it. You do not need to taste a shot to know which way to turn the grinder when it came in six seconds short. So the clock advises on its own, and taste *upgrades* that advice rather than unlocking it: the one case the clock cannot see — quick and already bitter, which is a channel and not a coarse grind — is named in the copy, which is a better argument for using the taste scale than withholding the whole answer was.

Where a wall has been named and the clock is decisive, the clock stands in for the axis nobody rated and the four corners resolve exactly as they do from taste alone. The copy says where the reading came from: nobody said "sour", the timer did.

The exception is the one the kit exists to catch. In an immersion brewer the time is a number somebody chose, so there is nothing in it to diagnose, and the brew log says that instead of inventing a symptom.

**The answer goes where an answer goes.** A dial-in is a question — *what do I change now?* — and its answer used to be the last line of the newest record card, below the numbers, the window, the deltas and the taste pips. It is a card at the top of the board now, headed **Next**, and it is not repeated underneath: one answer, one place. Before the first record it holds the starting point instead — dose, window, and the only honest thing anyone can say about a grind setting, which is that the number is meaningless on someone else's grinder and the window tells you which way to move. A board that gives the starting point needs no "nothing here yet" underneath it.
...
**Work in progress outranks the form that would replace it.** The setup screen has two jobs — join or set one up — and after a reload mid-cupping it was doing neither of the ones that mattered: Resume sat 910px down an 844px viewport, under the whole new-cupping form, so a cupper who reloaded saw a door into somebody else's table and a form for starting a rival one, and nothing above the fold even said they were in a cupping. A session in progress goes at the top, in its own card. The state goes in the card and not in the button label, because "Resume · 1 of 4 finished · 1 part-scored" is a forty-character button.

**Your own table is not a stranger's.** Typing the code you are already sitting at is what somebody does when they cannot find the way back in. The app read it as joining a stranger, offered to discard the session, and did exactly that if you agreed — returning you as a second seat under your own name that quietly moved the panel average. Re-entering your own code resumes; there is nothing to confirm because there is nothing to replace.

**A comparison needs a column.** The cupper rows under each panel score were inline-block chips inside a line-height, so they reflowed as running text: two cuppers on the first line, one on the second, the numbers landing wherever the names happened to end. At a table whose whole point is comparing your number to everyone else's, there was no column to run an eye down. One cupper per row, name / score / difference, scores right-aligned in tabular figures.

**Help does not stop at the door of Results.** Every loaded word in this product has a "?" beside it, and a first-time cupper said that was the thing that made the sheet survivable. Then she reached Results — "panel", "calibration", "average difference from the panel", "±1.5" — and there was not one help mark on the screen. The scaffolding disappeared exactly where somebody new is most exposed: in front of other people, reading a number about their own palate.
