...
The part of it a browser can read lives in **`public/shared/tokens.css`**: palette for both themes, the two faces, the spacing scale, radius and easing. Every app links it before its own stylesheet and precaches it in its own service worker. A second copy of the palette is a second palette inside a month, and the figures in this document were measured once.

**The component layer is now the same argument, and it has not been made yet.** The brew log's stylesheet is the dial-in's, adapted — deliberately, because two instruments on the same bench should be built from the same parts and a brewer who uses both should not have to learn a second set of controls. But that is a third copy of `.btn`, `.num-field`, `.modal-sheet` and `.scale` in the repository, and three copies drift exactly the way two palettes would. The next structural move on this codebase is to lift the shared components out into `public/shared/components.css`, leaving each app only what is actually its own. It is written here rather than done in the same commit as a new app because it touches two shipped ones, and a refactor of everything is not a thing to bundle with a build of something.
